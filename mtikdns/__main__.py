...
class InterceptResolver(BaseResolver):
    ...
    def resolve(self,request,handler):
        ...
        if qname.label[-1] == b"local" and qtype == 'A': # check the mikrotik
            leases = { x['host-name'].lower() if "host-name" in x else "":x['address'] for x in api.get_resource('/ip/dhcp-server/lease').get()}
            requested_name = qname.label[-2].decode("utf-8").lower()
            if requested_name in leases:
                reply.add_answer(*RR.fromZone(f"{str(qname)} A {leases[requested_name]}"))
                return reply
...
if __name__ == '__main__':
    ...
    p.add_argument("--username",default="admin",
                help="Mikrotik username (default: admin)")
    p.add_argument("--password",default="",
                help="Mikrotik pasword (default: )")
    p.add_argument("--router",default="192.168.0.1",
                help="Mikrotik address (default: 192.168.0.1)")
    args = p.parse_args()
    ...
    connection = routeros_api.RouterOsApiPool(args.router, username=args.username, password=args.password, plaintext_login=True)
